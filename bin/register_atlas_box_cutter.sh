...
args() {
    ...
    if [ -z ${BOX_CUTTER_ATLAS_USERNAME+x} ]; then
...
publish_provider() {
    ...
    HTTP_STATUS=$(curl -s -f -o /dev/nul -w "%{http_code}" -i "${ATLAS_API_URL}/box/${BOX_CUTTER_ATLAS_USERNAME}/${BOX_NAME}/version/${VERSION}/provider/${PROVIDER}"?access_token="${BOX_CUTTER_ATLAS_ACCESS_TOKEN}" || true)
    echo ${HTTP_STATUS}
    if [ 200 -eq ${HTTP_STATUS} ]; then
        echo "==> Updating ${PROVIDER} provider"
        curl -X PUT "${ATLAS_API_URL}/box/${BOX_CUTTER_ATLAS_USERNAME}/${BOX_NAME}/version/${VERSION}/provider/${PROVIDER}" -d "access_token=${BOX_CUTTER_ATLAS_ACCESS_TOKEN}" -d provider[name]="${PROVIDER}" -d provider[url]="${PROVIDER_URL}"
    else
        echo "==> Creating ${PROVIDER} provider"
        curl -X POST "${ATLAS_API_URL}/box/${BOX_CUTTER_ATLAS_USERNAME}/${BOX_NAME}/version/${VERSION}/providers" -d "access_token=${BOX_CUTTER_ATLAS_ACCESS_TOKEN}" -d provider[name]="${PROVIDER}" -d provider[url]="${PROVIDER_URL}"
    fi
}
...
main() {
    ...
    # Retrieve version
    HTTP_STATUS=$(curl -s -f -o /dev/nul -w "%{http_code}" -i "${ATLAS_API_URL}/box/${BOX_CUTTER_ATLAS_USERNAME}/${BOX_NAME}/version/${VERSION}" || true)
    ...
    if [ 404 -eq ${HTTP_STATUS} ]; then
       ...
       JSON_RESULT=$(curl -s -f -X POST -H "Content-Type: application/json" "${ATLAS_API_URL}/box/${BOX_CUTTER_ATLAS_USERNAME}/${BOX_NAME}/versions?access_token=${BOX_CUTTER_ATLAS_ACCESS_TOKEN}" -d "${VERSION_JSON}" || true)
    else
    ...
    BOXCUTTER_BASE_URL=http://cdn.boxcutter.io/debian
    if [[ -e ${VMWARE_BOX_FILE} ]]; then
    ...
    case $STATUS in
    unreleased)
      curl -X PUT "${ATLAS_API_URL}/box/${BOX_CUTTER_ATLAS_USERNAME}/${BOX_NAME}/version/${VERSION}/release" -d "access_token=${BOX_CUTTER_ATLAS_ACCESS_TOKEN}"
      echo 'released!'
      ;;
    active)
